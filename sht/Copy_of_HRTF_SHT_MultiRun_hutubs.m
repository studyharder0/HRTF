...
path = 'sofa\';
SH_order = 26;
hrtf_freq_allDB = zeros(32, 128, 1250, 2);
hrtf_SHT_dBmat = zeros(32, 128, (SH_order+1)^2, 2);

% hrtf_dir = dir([path,'*measured.sofa'])

hrtf_dir = dir([path,'*.sofa']);
...
hrtf_freq_l = fft( squeeze(hrtfData.Data.IR(:,1,:) )',256 );
hrtf_freq_r = fft( squeeze(hrtfData.Data.IR(:,2,:) )',256 );
...
%     C_matrix_r(:,i_freq) = C2;
    ...
    hrtf_freq_allDB(ind, i_freq, :, 1) = f1;
    hrtf_freq_allDB(ind, i_freq, :, 2) = f2;
