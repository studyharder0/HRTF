...
input_hrtf = fft( squeeze(hrtfData.Data.IR(:,1,:) )',256);
...
N = length(TH);
freq_vec = linspace(0, 44100/2, 128);
...
%plotSphFunctionTriangle(f, [TH,PHI], 'complex');
...
%legend('个体010真实值','个体003真实值','location','best', 'FontName','宋体', 'FontSize', 12)

xlabel('Direction') 
axis([0 1250,0,4])
ylabel('HRTF Magnitude(dB)')
title(strcat('L=26 RMSE=',num2str(error_rms)))
...
%plotSphFunctionTriangle(abs(Recons_error), [TH,PHI]);
...
%subplot(233); 
plot(C);
xlabel('球谐基索引','FontName','宋体', 'FontSize', 12) 
ylabel('球谐系数','FontName','宋体', 'FontSize', 12)
axis([0,25,-2,2])
